...
public class TableData extends DateOperations {
	...
	public static List<String> getRowDatas(WebElement targetTable, int rowIndex) {
	    List<String> rowData = new ArrayList<>();
	    int i = 0, colCount = getColumnCount(targetTable);
	    
	    while (i < colCount) {
	        WebElement cellElement = getElement(targetTable, rowIndex, i);
	        
	        // Check if the element is not null before trying to get text
	        if (cellElement != null) {
	            String cellText = cellElement.getText();
	            System.out.println("CellText : " + cellText);
	            rowData.add(cellText);
	        } else {
	            System.out.println("Cell element is null for row " + rowIndex + ", column " + i);
	            // You might want to add a placeholder or handle the null case as needed
	            rowData.add("N/A");
	        }
    ...
    public static int getRowCount_css( WebElement table) {
        List<WebElement> rows = table.findElements(By.cssSelector("tbody > tr"));
        return rows.size();
    }

    public static int getColumnCount_css( WebElement table) {
        List<WebElement> headerCells = table.findElements(By.cssSelector("thead > tr > th"));
        return headerCells.size();
    }

    public static String getTableCellText_css( WebElement table, int row, int column) {
        WebElement cell = table.findElement(By.cssSelector("tbody > tr:nth-child(" + row + ") > td:nth-child(" + column + ")"));
        return cell.getText();
    }
	
    public static ArrayList<String> getRowText_css(WebElement table, int rowIndex) {
        List<WebElement> rowCells = table.findElements(By.cssSelector("tbody > tr:nth-child(" + rowIndex + ") > td"));
        ArrayList<String> cellTexts = new ArrayList<>();
...
//    methods to deal with the main table on process RFQ screen
	...
	public static List<String> getTextFromTrElements(WebElement table, String className, int row) {
